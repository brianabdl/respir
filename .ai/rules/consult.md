...
## Cough is async 202 + Reverb; consent endpoint added

Update to earlier contract notes: CoughAnalysisAgent no longer exists (deleted). POST consult/{consultation}/cough now stores the capture and returns 202 {status:processing, capture_id}; the structured analysis arrives later via Reverb event on private consultation.{id} (CoughAnalysisCompleted). POST consult/{consultation}/consent records consented_at (ConsentRequest authorizes owner). Validation errors for cough still come from CoughSampleRequest. Voice/chat/live-token behavior is unchanged and still Gemini-backed; chat/voice/sessionLog/capture authorization lives in Form Requests or authorizeConsultation().

## Anemia screening pipeline + Live cue contract
Anemia screening: POST consult/{id}/anemia stores a palm/eye/nail capture and returns 202; AnalyseAnemia (ai queue) calls PythonAiClient::analyzeAnemia -> /v1/vision/anemia (multipart field `image` + `part`), persists analysis/risk_level/analyzed_at on consult_captures, and broadcasts `anemia.analysis` on private consultation.{id}. Consent enforced server-side; throttle consult-anemia 10/min. Auto-capture contract: the Live system instruction in ConsultationController::liveSystemInstruction must keep the exact cues "I'm ready to capture. Hold your palm ... / Pull your lower eyelid ... / Hold one fingernail ...", because resources/js/pages/consult.tsx regex-matches that phrase to snapshot the camera. Change one side, change the other.
